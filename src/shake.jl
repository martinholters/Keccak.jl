for d in [128, 256]
    ...
    @eval begin
        ...
        The provided `data` must be an `AbstractVector` or a `Tuple` of `UInt8`s.
        Appropriate padding is applied by `$($shakefunc)`.
        """
        function $shakefunc(data::Union{AbstractVector{UInt8},NTuple{<:Any,UInt8}})
            sponge = $spongefunc()
        ...
        is returned as a tuple of `UInt8`s if `len` is a `Val`, as a `Vector{UInt8}`
        otherwise.
        ...
        function $shakefunc(args::Vararg{Any,N}) where N
            data = args[1:end-1]::NTuple{N-1,Union{AbstractVector{UInt8},NTuple{<:Any,UInt8}}}
            len = args[end]
